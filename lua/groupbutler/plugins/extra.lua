...
function _M:onTextMessage(blocks)
	...
	if blocks[1] == 'add_notes' then
		...
		if msg.reply and not blocks[3] then
			...
			api:editMessageText(msg.from.chat.id, msg_id, nil, i18n("Notes %s saved!"):format(blocks[2], '`, `'))
		end
	elseif blocks[1] == 'notes' then
		...
		elseif blocks[1] == 'remove' then
			...
			local text = i18n("Notes deleted: `%s`"):format(table.concat(deleted, '`, `'))
			if next(not_found) then
				text = text..i18n('\nNotes not found: `%s`'):format(table.concat(not_found, '`, `'))
			end
