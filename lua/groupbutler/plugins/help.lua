...
I'm Miss Jessie the first administration bot using the official Bot API.
...
• `/tempban` : ban a user for a specific amount of time. Use the returned keyboard to ban the user.
...
• `/fwdban` : ban the original sender of a forwarded message.
• `/unban` : unban the user from the group.
• `/user` : shows how many times the user has been banned *in all the groups*, and the warns received.
• `/status` : show the current status of the user `(member|restricted|kicked/left the chat|banned|admin/creator|never seen)`.
...
`$rules`: _will be replaced by a link to the rules of the group._
...
The bot auto-removes _https://, http:// and www_ from every link to reduce the possibility of having the same link saved twice.]]), -- luacheck: ignore 631
		extra = i18n([[*Notes*
#notes commands are a smart way to save your own custom commands.

• `/add_notes [#notename] [note]`: set a reply to be sent when someone writes the trigger.
_Example_ : with "`/add_notes #hello Good morning!`", the bot will reply "Good morning!" each time someone writes #hello.
You can reply to a media (_photo, file, vocal, video, gif, audio_) with `/add_notes #yourtrigger` to save the #notes and receive that media each time you use # command
• `notes`: get the list of your custom commands.
• `/remove [#notename]`: delete the trigger and its message.
